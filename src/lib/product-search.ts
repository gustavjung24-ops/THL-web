...
type VariantConfidence = "verified" | "suggested" | "manual_review";

type BrandSuffixPolicyConfig = {
  allowedSuffixes?: string[];
  blockedAutoGeneratedSuffixes?: string[];
  rule?: string;
  confidenceDefault?: VariantConfidence;
};

type BrandSuffixPolicyMap = Record<string, BrandSuffixPolicyConfig>;

const brandSuffixPolicy = brandSuffixPolicyData as BrandSuffixPolicyMap;
...
function getPolicyKeyForBrand(brand: ProductBrand): string {
  if (brand === "Koyo") return "KOYO/JTEKT";
  if (brand === "Soho V-Belt") return "SOHO";
  return brand;
}

function getPolicyForBrand(brand: ProductBrand): BrandSuffixPolicyConfig | null {
  const policyKey = getPolicyKeyForBrand(brand);
  return brandSuffixPolicy[policyKey] ?? null;
}

function parseCodeList(rawValue: unknown): string[] {
...
function extractSuffix(baseProductCode: string, productCode: string): string {
  const baseCompact = normalizeCompact(baseProductCode);
  const codeCompact = normalizeCompact(productCode);

  if (!baseCompact || !codeCompact || codeCompact === baseCompact) {
    return "";
  }

  if (codeCompact.startsWith(baseCompact)) {
    return codeCompact.slice(baseCompact.length);
  }
...
function classifyConfidence(input: {
  ...
  defaultConfidence: VariantConfidence;
}): VariantConfidence {
  const {
    isVariant,
    source,
  ...
  if (Array.from(blockedSuffixes).some((blocked) => suffixCompact.includes(blocked))) {
    return "manual_review";
  }

  if (allowedSuffixes.size > 0 && !allowedSuffixes.has(suffixCompact)) {
    return "manual_review";
  }
...
async function loadSearchIndex(): Promise<ProductSearchIndex> {
  ...
  const missingPolicyBrands = new Set<string>();
  ...
  for (const fileName of fileNames) {
    ...
    for (const product of rawProducts) {
      ...
      const source = toStr(product.source);
      const sourceUrl = toStr(product.source_url);
      ...
      const sourceVerifiedVariantCompacts = new Set(
        parseCodeList(product.source_verified_variants)
          .map(normalizeCompact)
          .filter(Boolean),
      );
      const policy = getPolicyForBrand(brand);
      if (!policy) {
        missingPolicyBrands.add(brand);
      }
      const blockedSuffixes = new Set((policy?.blockedAutoGeneratedSuffixes ?? []).map(normalizeCompact).filter(Boolean));
      const allowedSuffixes = new Set((policy?.allowedSuffixes ?? []).map(normalizeCompact).filter(Boolean));
      const defaultConfidence = policy?.confidenceDefault ?? "manual_review";
  ...
  if (missingPolicyBrands.size > 0) {
    console.warn(`[product-search] Missing brand suffix policy for: ${Array.from(missingPolicyBrands).join(", ")}`);
  }
...
function computeCandidate(item: IndexedProduct, queryCompact: string, queryTokens: string[]): SearchCandidate | null {
  ...
  if (exactAlias) {
    ...
    return {
      item,
      matchTier: 3,
      matchedBy: item.confidence === "manual_review" ? "Khớp alias (cần xác minh)" : "Khớp alias dữ liệu",
