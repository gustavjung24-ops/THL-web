...
export async function listProactiveQuotes(): Promise<QuoteRequestRecord[]> {
  const quotes = await listQuoteRequests();
  return quotes.filter((item) => item.sourceType === "manual");
}
...
export async function getProactiveQuoteById(id: string): Promise<QuoteRequestRecord | null> {
  const quote = await getQuoteRequestById(id);
  if (!quote || quote.sourceType !== "manual") {
    return null;
  }
  return quote;
}
